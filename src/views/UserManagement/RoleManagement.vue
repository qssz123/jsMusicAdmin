<template>
  <el-card class="role-card">
    ...
    <!-- 表格 -->
    <el-table :data="filteredRoles" border style="width: 100%; margin-top: 20px">
...
// 角色列表
const roles = ref([
  { id: 1, name: '管理员', status: 'enabled', statusText: '启用', createdAt: '2025-10-01' },
  { id: 2, name: '编辑', status: 'enabled', statusText: '启用', createdAt: '2025-10-05' },
  { id: 3, name: '访客', status: 'disabled', statusText: '禁用', createdAt: '2025-10-10' }
])
...
const saveRole = () => {
  ...
  if (isEdit.value) {
    ...
    if (index !== -1) {
      roles.value[index].name = form.name
      roles.value[index].status = form.status
      roles.value[index].statusText = form.status === 'enabled' ? '启用' : '禁用'
      ElMessage.success('编辑成功')
    }
  } else {
    const newId = roles.value.length ? Math.max(...roles.value.map((i) => i.id)) + 1 : 1
    roles.value.push({
      id: newId,
      name: form.name,
      status: form.status,
